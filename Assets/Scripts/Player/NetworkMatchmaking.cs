...
public sealed class NetworkException : Exception
{
  ...
  public readonly NetworkExceptionType errorCode;

  public NetworkException(NetworkExceptionType errorCode, string msg)
  {
...
public class NetworkMatchmaking
{
  ...
  {
    textStatus = SceneData.Singleton.textGameStatus;
  ...
  private async Task RelayJoinAsClientAsync(string joinCode, int retryCount = 5, int delayMs = 500)
  {
    ...
    for (int attempts = 0; attempts < retryCount; attempts++)
    {
      ...
      catch (Exception ex)
      {
        ...
        throw new NetworkException(NetworkExceptionType.MultiplayFail, "Client Join Fail");
      }
    }

    throw new Exception("MATCHMAKING_STATUS | Join failed. Could not find relay code");
  }
